                ...
                @foreach ($checklists as $checklist)
                    <x-card title="{{ $checklist->name }}">
                        <div class="flex justify-between">
                            ...
                            <div>
                                <x-forms.form action="{{ route('checklists.restore', $checklist->id) }}"
                                    method="POST">
                                    @method('PUT')
                                    <x-secondary-button type="submit"
                                        title="{{ __('checklists.trash_restore_button') }}">
                                ...
                                <x-forms.form action="{{ route('checklists.force', $checklist->id) }}"
                                    method="POST">
                                    @method('DELETE')
                                    <x-danger-button type="submit" title="{{ __('checklists.force') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
